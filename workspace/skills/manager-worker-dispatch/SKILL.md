...
description: Use this skill to manage and dispatch tasks to workers. Triggers include: any request to break a task into subtasks and assign them to workers or bots; listing, creating, or selecting workers via the manager_worker_api; sending @-mention messages to workers inside a conversation or chat room; coordinating sequential or parallel multi-worker execution pipelines; dispatching frontend, backend, or QA work to the right worker by capability. Do NOT use for generic project planning, single-agent tasks.
---
...
6. Add the worker to the target conversation when needed.
7. Dispatch the subtask by having a bot send a message in that conversation, and make sure every manager-to-worker message starts with an `@` prefix, for example `@bob 你来写前端代码`.
...
python scripts/manager_worker_api.py join-worker --conversation-id room-123 --worker-id u-alex
python scripts/manager_worker_api.py send-message --bot-id u-manager --chat-id room-123 --text "@alex 你来进行测试，验证登录流程并记录回归风险"
python scripts/manager_worker_api.py ensure-and-dispatch --conversation-id room-123 --bot-id u-manager --name bob --description "frontend ui styling interaction" --task "你来写前端代码，实现设置页 UI" --dry-run
```
...
- Add the worker to the conversation before dispatching the task when the conversation does not already include that worker.
